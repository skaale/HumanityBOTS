<template>
  <div class="flex flex-col h-[calc(100vh-8rem)]">
    ...
    >
      ...
      <span class="text-[var(--text-tertiary)] text-sm">{{ codingBots.length }} bot{{ codingBots.length !== 1 ? 's' : '' }} coding</span>
    ...
    <div class="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 min-h-0">
      ...
      <div class="lg:col-span-4 flex flex-col min-h-0 space-y-4">
        <section class="card overflow-hidden flex flex-col min-h-0">
          <h3 class="section-title px-4 py-3 border-b border-[var(--border-subtle)]">Bot communication</h3>
          <div class="flex-1 overflow-auto p-2 space-y-2 min-h-[140px] max-h-[200px]">
            <div
              v-for="msg in stream.botMessages.slice().reverse().slice(0, 25)"
              :key="msg.id"
              class="text-xs py-2 px-3 rounded-md bg-[var(--bg-primary)] border-l-2 border-[var(--accent)]"
            >
              <div class="flex items-center gap-1 text-[var(--accent)] font-medium">
              ...
              <time class="text-[10px] text-[var(--text-tertiary)]">{{ formatTs(msg.ts) }}</time>
            </div>
          </div>
          <p v-if="!stream.botMessages.length" class="text-xs text-[var(--text-tertiary)] px-3 py-2">Bots will show messages here. Use header toggles to hear bot talk and edit sounds.</p>
        </section>
        <section class="card overflow-hidden flex flex-col min-h-0">
          ...
          <p v-if="!agentsWritingCode.length" class="text-xs text-[var(--text-tertiary)] px-3 py-2">No agent contributions yet. Names appear here and in the editor gutter as they write.</p>
        </section>
        <section class="card overflow-hidden flex flex-col min-h-0">
          ...
          <div class="flex-1 overflow-auto p-2 space-y-1 max-h-[200px]">
            ...
            >
              ...
              <span class="text-[var(--text-tertiary)] flex-shrink-0">{{ formatTs(edit.ts) }}</span>
            </div>
          </div>
        </section>
      </div>
